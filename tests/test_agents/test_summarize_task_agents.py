...
async def test_summarize_agent(monkeypatch, dummy_session, dummy_vector_results):
    ...
    async def fake_search(session, user_id, query_embedding, top_k, object_types, time_start, time_end, sources):
...
async def test_task_agent(monkeypatch, dummy_session, dummy_vector_results):
    ...
    result = await agent.detect_tasks(dummy_session, user_id="u1", prompt="what needs action")
