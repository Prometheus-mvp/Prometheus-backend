...
class LinkedAccount(Base):
    ...
    status = Column(Text, nullable=False, server_default="active")  # active | revoked | error
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint("user_id", "provider", "provider_account_id", name="uq_linked_accounts_user_provider_account"),
        Index("idx_linked_accounts_user_provider", "user_id", "provider"),
    )

    # Relationships
    user = relationship("User", back_populates="linked_accounts")
    oauth_tokens = relationship("OAuthToken", back_populates="linked_account", cascade="all, delete-orphan")
    events = relationship("Event", back_populates="source_account", foreign_keys="Event.source_account_id")
