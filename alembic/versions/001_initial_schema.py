...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Text(), nullable=False, server_default='active'),  # active | revoked | error
        sa.Column('metadata', postgresql.JSONB(), nullable=False, server_default='{}'),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.UniqueConstraint('user_id', 'provider', 'provider_account_id', name='uq_linked_accounts_user_provider_account'),
    )
    op.create_index('idx_linked_accounts_user_provider', 'linked_accounts', ['user_id', 'provider'])
    ...
    op.create_table(
        ...
        sa.Column('scopes', sa.Text(), nullable=True),
        sa.Column('access_token_enc', sa.Text(), nullable=False),  # AES-256-GCM encrypted
        sa.Column('refresh_token_enc', sa.Text(), nullable=True),  # AES-256-GCM encrypted
    ...
    op.create_index('idx_oauth_tokens_user_linked_account', 'oauth_tokens', ['user_id', 'linked_account_id'])
    op.create_index('idx_oauth_tokens_user_id', 'oauth_tokens', ['user_id'])
    op.create_index('idx_oauth_tokens_linked_account_id', 'oauth_tokens', ['linked_account_id'])
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.UniqueConstraint('user_id', 'source', 'external_id', name='uq_threads_user_source_external'),
    )
    op.create_index('idx_threads_user_source_last_event', 'threads', ['user_id', 'source', 'last_event_at'])
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('source', sa.Text(), nullable=False),  # slack | telegram | outlook | calendar
        sa.Column('source_account_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('linked_accounts.id', ondelete='SET NULL'), nullable=True),
        sa.Column('external_id', sa.Text(), nullable=False),
        sa.Column('thread_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('threads.id', ondelete='SET NULL'), nullable=True),
        sa.Column('event_type', sa.Text(), nullable=False),  # message | reaction | mention | file | email | calendar_created etc.
        sa.Column('title', sa.Text(), nullable=True),
        sa.Column('body', sa.Text(), nullable=True),
        sa.Column('url', sa.Text(), nullable=True),
        sa.Column('text_for_embedding', sa.Text(), nullable=True),
        sa.Column('content_hash', sa.Text(), nullable=False, server_default=''),
        sa.Column('importance_score', sa.SmallInteger(), nullable=False, server_default='0'),
        ...
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.UniqueConstraint('user_id', 'source', 'external_id', name='uq_events_user_source_external'),
    )
    op.create_index('idx_events_user_source_occurred', 'events', ['user_id', 'source', 'occurred_at'])
    ...
    op.create_index('idx_entities_user_kind_name', 'entities', ['user_id', 'kind', 'name'])
    ...
    op.create_table(
        ...
        sa.Column('priority', sa.Text(), nullable=False, server_default='medium'),  # low | medium | high
    ...
    op.create_index('idx_tasks_user_status_due', 'tasks', ['user_id', 'status', 'due_at'])
    ...
    op.create_index('idx_summaries_user_window', 'summaries', ['user_id', 'window_start', 'window_end'])
    ...
    op.create_index('idx_proposals_user_window', 'proposals', ['user_id', 'window_start', 'window_end'])
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('kind', sa.String(50), nullable=False),  # slack_reply | telegram_reply | outlook_reply | note
        sa.Column('content_json', postgresql.JSONB(), nullable=False),
    ...
    op.create_index('idx_drafts_user_kind_created', 'drafts', ['user_id', 'kind', 'created_at'])
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('object_type', sa.String(50), nullable=False),  # event | note | thread | draft | entity
        sa.Column('object_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('chunk_index', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('embedding_model', sa.String(100), nullable=False),
        sa.Column('embedding_dim', sa.Integer(), nullable=False),
        sa.Column('distance_metric', sa.String(50), nullable=False, server_default='cosine'),
        sa.Column('embedding', Vector(1536), nullable=False),
        sa.Column('content_hash', sa.String(255), nullable=False),
        sa.Column('metadata', postgresql.JSONB(), nullable=False, server_default='{}'),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.UniqueConstraint('user_id', 'object_type', 'object_id', 'chunk_index', 'embedding_model', name='uq_embeddings_user_object_chunk_model'),
    )
    op.create_index('idx_embeddings_user_object_type', 'embeddings', ['user_id', 'object_type'])
    op.create_index('idx_embeddings_user_content_hash', 'embeddings', ['user_id', 'content_hash'])
